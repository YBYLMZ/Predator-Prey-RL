...
class Quadrotor:
    ...
    def __init__(self, state0, coeff_pos=1.0, coeff_angle=0.25, coeff_control=0.0, coeff_final_pos=0.0):
        ...
        self.Controllers = ["Backstepping_1",
                            "Backstepping_2", "Backstepping_3", "Backstepping_4"]
    ...
    def model_dynamics_2(self, t, state):
        ...
        state_dot[9] = theta_dot*psi_dot*I1 - \
            Jr / Ixx * theta_dot * omega + l/Ixx*U2
        state_dot[10] = phi_dot*psi_dot*I2 + \
            Jr / Iyy * phi_dot * omega + l/Iyy*U3
    ...
    #                 xd_ddddot[i], yd_ddddot[i], psid[i], psid_dot[i], psid_ddot[i]]
        ...
        x1, x2, x3 = array([[x], [y]]), array(
            [[x_dot], [y_dot]]), array([[phi], [theta]])
        x4, x5, x6 = array([[phi_dot], [theta_dot]]), array(
            [[psi], [z]]), array([[psi_dot], [z_dot]])
        ...
        dl0_dx3 = array([[-np.sin(phi)*np.sin(theta), np.cos(phi)
                        * np.cos(theta)],  [np.cos(phi), 0]])*U1/m
        dl0_dx3_inv = array([[0, 1/np.cos(phi)],  [1/np.cos(theta) *
                            1/np.cos(phi), 1/np.cos(phi)*np.tan(theta)*np.tan(phi)]])*m/U1
    ...
    #     h1 = [-Jr/Ixx*theta_dot*omega  Jr/Iyy*phi_dot*omega]
        ...
        v3_dot = dot(dl0_dx3_inv, dot(g0.T, z2_dot) + v2_ddot + dot(A3, z3_dot)
                     ) + dot(dl0_dx3_inv_dot, dot(g0.T, z2) + v2_dot + dot(A3, z3))
        l1 = dot(k1_inv, dot(dl0_dx3.T, z3) +
                 v3_dot - g1 - h1 + dot(A4, z4)).ravel()
    ...
    def get_control_input(self, state, U0, cont, current_traj):
        if (cont == self.Controllers[0]):  # Backstepping_1
            ...
            U = self.backstepping(A1, A2, A3, A4, A5, A6,
                                  state, U0, current_traj)
    ...
    def simulate(self, current_traj, dtau=1e-3, method="Backstepping_3"):
        ...
        # Euler Solver
        self.U = self.get_control_input(
            self.state, self.U, method, current_traj)
